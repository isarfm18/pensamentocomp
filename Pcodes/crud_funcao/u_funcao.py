...
class AlterarFuncao:
  def __init__(self, janela_mestre, idt):
      ...
      lb_nome = tk.Label(self.popup, text="Nome do Função", font='Helvetica 12 bold', fg='blue')
      ...
      lb_vlrdiafuncao = tk.Label(self.popup, text="Valor da diaria da função", font='Helvetica 12 bold', fg='blue')
      lb_vlrdiafuncao.grid(row=4, column=0, padx=PADX, pady=PADY)
  ...
  def alterar(self, janela_mestre):
      ...
      if retorno[0]:
          ...
          cmd = "UPDATE tb_funcao SET nme_funcao = %s, vlr_dia_funcao = % WHERE idt_funcao = %s"
          num_reg = janela_mestre.sql.upd_del(cmd, (nome, vlr_dia_funcao, idt))
