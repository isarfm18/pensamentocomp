...
class CRUDProjeto(tk.Tk):
  def __init__(self):
      ...
      self.tre_funcoes = ttk.Treeview(self, columns=("idt", "nme", "dta_ini_projeto"), show="headings", style="Custom.Treeview")
      # Configurar as colunas
      self.tre_funcoes.heading("idt", text="Identificador")
      self.tre_funcoes.heading("nme", text="Nome do Projeto")
      self.tre_funcoes.heading("dta_ini_projeto", text = "Data de Início")
      # Ajustar a largura das colunas
      self.tre_funcoes.column("idt", width=100)
      self.tre_funcoes.column("nme", width=400)
      self.tre_funcoes.column("dta_ini_projeto", width=150)
  ...
  def consultar(self):
      ...
      projetos = self.sql.get_list(cmd, [nome])
      ...
      for projeto in projetos:
          # Formatar a data
          dta_ini_projeto = projeto['dta_ini_projeto'].strftime("%d/%m/%Y")
          self.tre_funcoes.insert("", tk.END, values=(projeto['idt_projeto'], projeto['nme_projeto'], dta_ini_projeto))
